...
namespace Pos.Hr.PL
{
    public partial class Leave_Request_Approved : System.Web.UI.Page
    {
        ...
        private void getmaxgrade()
        {
           ...
           Session["comid1"].ToString() + "' and [Flows02].[cCompany]='" + Session["comid1"].ToString() + "' ", sqlcon);
            adapter3.Fill(dt2);
            if (dt2.Rows.Count > 0)
            {
                grademax = Convert.ToInt32(dt2.Rows[0]["grade"].ToString().Trim());
            }
        }
        private void needdatasow()
        {
            ...
            da = new SqlDataAdapter(" select DISTINCT [Hr11Leave].[cCompany],[Hr11Leave].[cRequestId],[Hr11Leave].[cRequestDate],[Hr11Leave].[cRequestFor],[Hr11Leave].[cRequestTopic],[Hr11Leave].[cVacationStart],[Hr11Leave].[cVacationEnd],[wfaut101].[cParam] from [Hr11Leave],[wfaut101],[Hr11Leave01] where ([Hr11Leave01].cApproved=1 AND [Hr11Leave01].cRequestId=[Hr11Leave].cRequestId AND [wfaut101].cRequestId=[Hr11Leave01].cRequestId ) UNION  select DISTINCT [Hr11Leave].[cCompany],[Hr11Leave].[cRequestId],[Hr11Leave].[cRequestDate],[Hr11Leave].[cRequestFor],[Hr11Leave].[cRequestTopic],[Hr11Leave].[cVacationStart],[Hr11Leave].[cVacationEnd],[wfaut101].[cParam] from [Hr11Leave],[Hr11Leave01],[wfaut101] where [Hr11Leave01].cRequestId=[Hr11Leave].cRequestId and [Hr11Leave01].cApproved=1  order by Hr11Leave.cRequestId desc   ", sqlcon);
                da.Fill(dt3);
        ...
        {
            Response.Redirect("../../WorkFlow/Reports/LeaveReportEntitlement.aspx");
        }
    }
}